...
export const getSelectedItemPropKey = <T extends StateBase>(target: T) =>
  target[selectedItemProp as keyof T] as keyof T | undefined;

export const getSelectedItemPropOptionsKey = <T extends StateBase>(key: string, target: T) =>
  target[selectedItemPropOptions(key) as keyof T] as keyof T | undefined;
